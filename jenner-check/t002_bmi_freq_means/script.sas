/* --------------------------------------------------------
   Task 9-10 (hw1.sas): label the variables, derive BMI
   (pounds->kg, inches->meters), classify it into CDC
   categories, flag diabetes status from glyhb, then check the
   new variables with PROC FREQ and PROC MEANS.

   Adapted for a self-contained run: the source table is
   reconstructed inline from a 45-row sample of the author's
   real diabetes.csv (Willems et al., 1997) rather than read
   from the Viya library hw1.diab2. The BMI formula, the CDC
   cut-points, the glyhb threshold and the PROC FREQ / PROC
   MEANS calls are the author's, unchanged.
   -------------------------------------------------------- */
...
data diab2;
    ...
    /* Categorical BMI variable (CDC guidelines) */
    length BMI_cat $ 15;
    ...
    /* Diabetes status based on glyhb */
...
proc freq data = diab2;
    tables BMI_cat diabetes;
    title "Frequency of BMI Category and Diabetes Status";
run;

proc means data = diab2 n mean min max;
    var BMI;
