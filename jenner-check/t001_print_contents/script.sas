/* --------------------------------------------------------
   Task 8 (hw1.sas): print the first 3 observations and
   inspect the contents of the imported diabetes table.

   Adapted for a self-contained run: instead of importing the
   diabetes.csv from the SAS Viya library (hw1.diab_tzavellas),
   the table is reconstructed inline from a 45-row sample of
   the author's real diabetes.csv (Willems et al., 1997). The
   PROC PRINT (obs=3) and PROC CONTENTS calls are the author's,
   unchanged.
   -------------------------------------------------------- */
...
proc print data = diab_&name. (obs=3);
    title "First 3 Observations of diab_&name.";
run;

proc contents data = diab_&name.;
    title "Contents of diab_&name.";
run;
